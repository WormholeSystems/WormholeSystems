...
const { map_characters } = defineProps<{ map_characters: TCharacter[] }>();
...
watch(
    ...
    (new_solarsystem_id, old_solarsystem_id) => {
        ...
        if (!old_solarsystem_id || !new_solarsystem_id) return;
        if (new_solarsystem_id === old_solarsystem_id) return;

        requestSolarsystemAdd(old_solarsystem_id, new_solarsystem_id);
    },
);

function requestSolarsystemAdd(old_solarsystem_id: number | null, new_solarsystem_id: number) {
    ...
    router.post(route('tracking.store'), {
